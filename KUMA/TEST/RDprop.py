...
class RDprop:
	...
	def getARD(self):
		...
		# Set distribution on crystal 
		gauss_rd.setTotalArea(self.beamsize)
		...
		intend=three_sigma 
		print "RANGERANGE",intstart,intend
...
if __name__=="__main__":
	...
	rdp=RDprop(beamsize,step)
	
	#rdp.virtualCrystal(100)
	print "MaxRD= %8.5f"%rdp.getARD()

	#ard,trans,thickness=rdp.getAttFac(1.0,1,1.0)
	#print "ARD:%8.1f %8.4f %8.0f[um]\n"%(ard,trans,thickness)

        #bs_list=[1.0,2.0,5.0,10.0]
        #hs_list=[0.5,1,2,5,10,20]
        #hs_list=arange(0.5,15.5,0.5)
	#for beamsize in bs_list:
		#for hstep in hs_list:
			#rdp.setBeamsize(beamsize)
			#rdp.setHstep(hstep)
			#print "%5.1f[um] %5.1f[um] MAXRD= %8.3f"%(beamsize,hstep,rdp.getARD())
