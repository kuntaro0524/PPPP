...
class NeedlePicture:
    ...
    def getCenterFWHM(self):
        ...
        n_pix = len(newy)  # Temporary debugging for deleting header strange pixels
        # Mean values are put into 6 pixels on the top
        # 2019/12/8 K.Hirata
        tmp_y_array = array(newy)
        mean_value = tmp_y_array.mean()

        # Trimming top of 8 pixels
        for i in range(0, 8):
            newy[i] = mean_value
        for i in range(n_pix - 8, n_pix):
            newy[i] = mean_value
