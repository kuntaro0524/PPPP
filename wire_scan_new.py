...
## 	TCS aperture list
	# Itsumono
	# Normally used
	tcs_list=[(0.026,0.040),(0.1,0.1),(0.3,0.3),(1.0,1.0)]

## 	Log file
        logname="%03d_wire.log"%(dev.f.getNewIdx3())
	logf=open(logname,"w")
...
## 	Counter
        counter_pin=Count(dev.getServer(),cnt_ch1,cnt_ch2)
...
#		time.sleep(900)
		...
		if dthetaTuneFlag:
			...
			# Delta theta1 tune
			if en > 10.0:
        			prefix="%03d_%s"%(dev.f.getNewIdx3(),prepre)
        			dev.mono.scanDt1PeakConfig(prefix,"DTSCAN_NORMAL",dev.tcs)
			elif en <= 10.0:
        			prefix="%03d_%s"%(dev.f.getNewIdx3(),prepre)
        			dev.mono.scanDt1PeakConfig(prefix,"DTSCAN_LOWENERGY",dev.tcs)
		...
		dev.gonio.moveXYZmm(sx,sy,sz)
        	prefix="%03d_colli"%(dev.f.getNewIdx3())
		...
		for tcs_param in tcs_list:
			...
			# PREFIX
       			prefix="%03d_%s_%s"%(dev.f.getNewIdx3(),prepre,tcsstr)
			...
			# photon flux
        		ipin,iic=counter_pin.getCount(1.0)
        		pin_uA=ipin/100.0
        		iic_nA=iic/100.0
