...
# 2015/02/09 K.Hirata for SACLA experiments
# This is finally for SACLA experiments : OOKUMA # MX225HS 
# 0: Normal
# 1: High gain
# 2: Low noise
# 3: High dynamic

class Schedule_HSS_Re:
	def __init__(self):
		self.dir="~/"
		self.dataname="test"
		self.offset=0
		...
		self.cl=200.0
		self.att_index=0
		self.isAdvanced=1 # SFROX Default = 1
		self.npoints=1
		self.astep=0
		self.ainterval=1
	...
	def setRotationCondition(self,startphi,endphi,stepphi):
	...
	def sfrox(self,svec,evec,startphi,stepphi,npoints,move_vec_length):
		# Shousuuten kiriage
		self.astep=move_vec_length/1000.0 # [um] -> [mm]
	
		self.ainterval=1 # 1 irradiation point = 1 frame
		self.scan_interval=1 # Successive rotaion range
		self.setAdvancedVector(svec,evec)
		# npoints
		...
		self.startphi=startphi
		# end phi
		self.endphi=self.startphi+self.stepphi*float(self.npoints)
	...
	def make(self,sch_file):
		...
		ofile.write("CCD Adc: 1  # 0:Normal  1:High gain 2:Low noise 3:Do not use (select Normal)\n")
