...
class DrawRectangleMouse:
	...
	def update(self,endx, endy, colour=SELECT_COLOUR):
		...
		cv2.rectangle(img, (self.x0, self.y0), (endx, endy), colour, 2)
		self.cenx=(self.x0+endx)/2.0
		self.ceny=(self.y0+endy)/2.0
		cv2.circle(img,(self.cenx,self.ceny),2,(0,0,255),2)
	...
	def show_cropped(self,event, x, y, flags, param):
		...
		if event == cv2.EVENT_LBUTTONDOWN:
			if self.isLocked==False:
				...
				self.isDrawing = False
				print "No space to save coordinates. Please click right button to clear."
