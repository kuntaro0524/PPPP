...
class LogStream:
    def __init__(self, write_func, write_stdout=False):
...
class xmlrpc_server(SimpleXMLRPCServer):
...
class MainFrame(wx.Frame):
    def __init__(self, parent=None, id=wx.ID_ANY, ms=None):
        wx.Frame.__init__(self, parent=parent, id=id, title="S. Centering", size=(500,500))
        self.intr = xmlrpc_server_interface(self, ms)
        
        vbox = wx.BoxSizer(wx.VERTICAL)
        hbox0 = wx.BoxSizer(wx.HORIZONTAL)
        self.btnStart = wx.Button(self, wx.ID_ANY, 'Start')
        self.btnStop = wx.Button(self, wx.ID_ANY, 'Stop')
        self.btnStart.SetFont(wx.Font(10, wx.FONTFAMILY_DEFAULT,
                                      wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD))
        self.btnStop.SetFont(wx.Font(10, wx.FONTFAMILY_DEFAULT,
                                      wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD))
        hbox0.Add(self.btnStart, 1, wx.GROW)
        hbox0.Add(self.btnStop, 1, wx.GROW|wx.LEFT)
        vbox.Add(hbox0, 0, wx.GROW)

        self.btnRun = wx.Button(self, wx.ID_ANY, 'Do centering')
        vbox.Add(self.btnRun, 0, wx.GROW)

        self.btnRunFacing = wx.Button(self, wx.ID_ANY, 'Do facing')
        vbox.Add(self.btnRunFacing, 0, wx.GROW)

        ## DEBUG
        hbox1 = wx.BoxSizer(wx.HORIZONTAL)
        self.btnPics = wx.Button(self, wx.ID_ANY, 'Take pics')
        hbox1.Add(self.btnPics)
        self.txtPics = wx.TextCtrl(self, wx.ID_ANY)
        hbox1.Add(self.txtPics)
        self.btnPics.Bind(wx.EVT_BUTTON, lambda e: self.intr.take_picstures_debug(int(self.txtPics.GetValue())))
        vbox.Add(hbox1, 0, wx.GROW)


        self.txtCtrl = wx.TextCtrl(self, wx.ID_ANY, style=wx.TE_MULTILINE|wx.TE_READONLY)
        vbox.Add(self.txtCtrl, 1, wx.GROW)

        self.btnQuit = wx.Button(self, wx.ID_ANY, 'Exit')
        vbox.Add(self.btnQuit)

        self.SetSizer(vbox)

        self.Show()
        self.Bind(wx.EVT_CLOSE, self.onClose)
        self.btnQuit.Bind(wx.EVT_BUTTON, self.onClose)
        self.btnStart.Bind(wx.EVT_BUTTON, self.BtnStart_onclick)
        self.btnStop.Bind(wx.EVT_BUTTON, self.BtnStop_onclick)
        self.btnRun.Bind(wx.EVT_BUTTON, self.BtnRun_onclick)
        self.btnRunFacing.Bind(wx.EVT_BUTTON, self.BtnRunFacing_onclick)

        self.logstream = LogStream(self.txtCtrl.AppendText, True)
    # __init__()
    ...
    def BtnStart_onclick(self, event):
        self.intr.capture.prep()
        self.intr.capture.setShutterSpeed(50)
        #self.intr.set_binning(4) # maximum view
        #self.intr.set_zoom(min(self.intr.coax_zoom2pulse.keys())) # minimum zoom <= why doesn't work?

    def BtnStop_onclick(self, event):
        self.intr.capture.disconnect()

    def BtnRun_onclick(self, event):
        self.intr.do_centering()

    def BtnRunFacing_onclick(self, event):
        self.intr.do_facing()
