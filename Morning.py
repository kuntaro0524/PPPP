...
class Morning():
    def __init__(self, path):
        ...
        # Capture setting
        # self.speed = 500
        #self.bright = 32768  # for WAT231S2 YK@200121
        self.bright = 3200  # YK@240318
    ...
    def stageYtuneCapture(self):
        ...
        self.tuneAttThick()
        print "Tuning attenuator finished."

        # Zoom in
        self.zoom.zoomIn()
        self.movePintZoomMax()

        # 190417 : this should be corrected. DANGER
        # Center cross in [pix]
        ceny = 320
        cenz = 240
        ...
        for i in range(0, 10):
            ...
            cap_file = "%s.ppm" % prefix
            print "Capturing %s" % cap_file
            capimg = self.cap.capture(cap_file)

            while (os.path.getsize(cap_file) != 881295):
                print "wait"
                time.sleep(0.5)

            print "Analyzing the image"
            bc = BeamCenter.BeamCenter(cap_file)
            y, z = bc.getGravCenter()

            # Before 2020/01/22
            # y,z=self.cap.aveCenter(prefix,avetime=5,bright=self.bright)      
            # controlled by exposure WAT231S2 YK@200121

            # diff x,y
            dy = y - ceny

            # pixel to micron [um/pixel] in high zoom
            # modified for CentOS6 (WAT231S2) by YK@200119
            # p2u_y=3.7215E-1 # till 201002.
            p2u_y = 0.44166
            # p2u_y=9.770E-2
    ...
    def stageYZtuneCapture(self):
        ...
        for i in range(0, 5):
            ...
            # y,z=self.cap.aveCenter(prefix,avetime=5,speed=self.speed)      # change by exposure DFK72 YK@190311
            y, z = self.cap.aveCenter(prefix, avetime=5, bright=self.bright)  # change by bright WAT231S2 YK@200121
    ...
    def getNeedleZcenter(self, filename):
        # 2019/04/05
        # self.cap.setBinning(1)
        #self.cap.capture(filename,5000)
        self.cap.capture(filename,2800) # 240118 YK
    ...
    def tuneCoaxZ(self, offset):
        ...
        for t in range(0, 20):
            ...
            print "CENTER ZURE=", diff
            # 2020/01/18 K.Hirata
            # This is strange value. Does this value fit to CentOS camera???
            #### pix2um_highz=0.07125   # [um/pixel]
            # 2020/01/18 K.Hirata
            # Newly evaluated value is 3.638 pixel/um thus 0.2714 um/pix
            # Evaluation
            # 1. Gonio move 10um step
            # 2. Capture the needle picture and analyze FWHM center at the horizontal center
            # 3. Fit a linear function to the observed plot and estimates this resolution of the camera
            # CentOS6, videosrv
            # modified for CentOS6 (WAT231S2) by YK@200119
            # pix2um_highz = 0.2714
            # modified for Ubuntu20 2020/10/02 K.Hirata
            #pix2um_highz = 0.44166
            # modified for Ubuntu20 2024/03/18 Y.Kawano
            pix2um_highz = 0.44150
            diff_um = diff * pix2um_highz
            print "Difference from center cross: %5.2f[um]" % diff_um
            ...
            if abs(diff_um) >= 0.5:
                ...
                print ("Convered! diff_z  =%8.3f um" % diff_um)
                break
    ...
    def changeAttCapture(self):
        ...
        for thick in self.att.getAttList():
            ...
            #self.cap.captureWithSpeed(tmpfile, 1000)  # change by exposure DFK72 YK@190311
            #self.cap.captureWithSpeed(tmpfile, 8000)  # change YK@240118
            self.cap.captureWithSpeed(tmpfile, 2800)  # change YK@240118
            bc = BeamCenter.BeamCenter(tmpfile)
    ...
    def tuneAttThick(self):
        ...
        for thick in self.att.getAttList():
            ...
            #self.cap.capture(tmpfile, 32768)  # 200121 same value at the header 'self.speed(bright) WAT231S2 YK@200121'
            #self.cap.capture(tmpfile, 8000)  # YK@240118'
            self.cap.capture(tmpfile, 2800)  # YK@240318'
            bc = BeamCenter.BeamCenter(tmpfile)
            ...
            # if aveall <= 60:
            if aveall <= 15:  # change threshold for WAT231S2 YK@200121
                threflag = True
    ...
    def aveCenter(self,prefix,avetime=5):
        totx=toty=0
        ...
        for i in range(0,avetime):
            ...
            time.sleep(0.5)
            pp=BeamCenter.BeamCenter(filename)
            x,y=pp.findRobust()
    ...
    def doCapAna(self, prefix, avetime=10, nrepeat=1, thicktune=True):
        ...
        for i in range(0, nrepeat):
            # y,z=self.cap.aveCenter(prefix,avetime=avetime,speed=self.speed)      # controlled by exposure DFK72 YK@190311
            y, z = self.aveCenter(prefix, avetime=avetime)
            sumy += y
            sumz += z
...
if __name__ == "__main__":
    ...
    filename = "/isilon/users/target/target/Staff/2022B/test01.ppm"
    fwhm, center = mng.getNeedleZcenter(filename)
    print fwhm, center
    # mng.tuneCoaxZ(0)
