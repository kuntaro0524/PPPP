...
class Stage:
	...
	def stageEvac(self):
		curry=self.getYmm()
		print "current Y position=%12.5f mm"%curry
		target_pos=curry+200.0
		self.setYmm(target_pos)
		aftery=self.getYmm()
		print "current Y position=%12.5f mm"%aftery
...
#		if diff_dist > 50.0:
	...
	def scanY(self,option="STAY",scan_step=0.01):
		f=File("./")
		curr_y=self.getY() #pulse

		# Hole diameter of coax-camera 1.5mm
		# 3mm width scan +-1.0mm
		width=3.0*self.p2v_y
		wing=int(width/2.0)

		# Scan step = 0.01mm
		scan_step_p=int(scan_step*self.p2v_y)
