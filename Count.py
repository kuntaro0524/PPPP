...
class Count:
	...
	def getStoredCount(self,time_msec):
		...
		if time < 0.1:
			msecTime=time*1000.0
			rtn_list=self.getCountMsec(msecTime)
			return rtn_list
...
if __name__=="__main__":
	...
	#ic,pin=counter.getCount(1.0)
	curr_phi=gonio.getPhi()
	#gonio.rotateNageppa(curr_phi+180.0)
	prev_time=datetime.datetime.now()
	for i in range(0,1):
		t=datetime.datetime.now()
        	ic,pin=counter.getCountMsec(1000)
		dt=t-prev_time
		print t,ic,pin,dt
		prev_time=t

	#print ic,pin
	#print ic/10.0,pin/100.0
