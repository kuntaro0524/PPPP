...
class AnaCBF:
    def __init__(self, cbf_numpy):
        self.imgdata = cbf_numpy
    ...
    def prep(self,prefix):
        ...
        sumdat.readSummary(prefix,nimages_all,1.00,timeout=self.timeout)
    ...
    def makeCheckMaps(self,prefix):
        ...
        kdtree_map_array=[]
        if self.debug:
            ofile=open("/tmp/check_map.dat","w")
        for v in range(0,self.nv):
            for h in range(0,self.nh):
    ...
    def searchPixelBunch(self,prefix,naname_include=True):
        ...
        # Logging
        if self.debug == True:
            logf=open("/tmp/log_%f.dat"%size_for_bunch,"w")
        crystal_array=[]
...
if __name__=="__main__":
    ...
    scan_path="/isilon/users/target/target/AutoUsers/190421/nanatani/CPS0261-01/scan00/2d/"
    scan_path="/isilon/users/target/target/AutoUsers/190519/abe//HSK0001-01/scan00//2d/"

    ahm=AnaHeatmap(scan_path,cxyz,phi)

    min_score=int(sys.argv[1])
    max_score=int(sys.argv[2])
    dist_thresh = float(sys.argv[3])
