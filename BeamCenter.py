...
class BeamCenter:
    ...
    def getGravCenter(self):
        # An original captured picture at the sample position
        orgimg = cv2.imread(self.filename, cv2.IMREAD_GRAYSCALE)

        # 2020/01/22 K.Hirata
        # When the threshold value is not suitable, the following routine
        # will do 'weak peak detection' by using 50 after 100.
        # I believe the value '100' is suitable to pick up the robust beam center.
        for thresh in [150, 100, 50]:
            print "Threshold=", thresh
            binimg = cv2.threshold(orgimg, thresh, 250,0)[1]
            cv2.imwrite("before_%d.png"%thresh, binimg)

            # A moment estimation of the beam position
            mu = cv2.moments(binimg, False)
            print mu
            if mu["m00"] != 0.0:
                x,y = int(mu["m10"]/mu["m00"]), int(mu["m01"]/mu["m00"])
                break
            else:
                print "Something wrong in getGravCenter.\n Check %s" % orgimg

            cv2.circle(binimg, (x,y), 4, 100, 2, 4)
            cv2.imwrite("ana_%d.png" % thresh, binimg)
    ...
    def gravAbove(self, image, thresh, two_flag=False):
        ...
        sumx = 0.0
        sumy = 0.0
        # Temporal code for BL32XU 2019/12/08
        n_remove = 10
        for x in range(n_remove, image.size[0]):
            for y in range(0, image.size[1] - n_remove):
                if pix[x, y] >= thresh:
                    if two_flag == False:
                        sumpeak += pix[x, y]
                        sumx += pix[x, y] * x
