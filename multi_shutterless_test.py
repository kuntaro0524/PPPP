...
# 2015/05/15 K.Hirata
# Multi-crystal data collection with shutterless measurement

class MultiCrystalShutteless:
	def __init__(self):
		self.dir="~/"
		self.dataname="test"
		self.offset=0
	...
	def make(self,sch_file,info_list):
		...
		for info in info_list:
			...
			dataidx+=1
			prefix,x,y,z=info
			self.dataname="%s%03d"%(prefix,dataidx)
...
if __name__=="__main__":
	...
	for line in lines:
		cols=line.split()
		if len(cols)==6:
			com,x,y,z=cols[1],float(cols[2]),float(cols[3]),float(cols[4])
			list="multi",x,y,z
	...
	# MX225HS 5 Hz setting
	hs_freq=5.0
	exposure_time_hs=1/hs_freq
	t.setExpTime(exposure_time_hs)

	# Best attenuation factor
	best_transmission=exptime/nframe*hs_freq
