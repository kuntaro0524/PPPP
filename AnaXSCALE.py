...
class AnaXSCALE():
    ...
    # lines_eachdata
    # includes "each data table" in XSCALE.LP
    def analyzeLineForEachData(self, lines_eachdata):
        ...
        bad_flag = False
        length_lines = len(lines_eachdata) 

        # Checkin Rsym value for each resolution shell
        for line in lines_eachdata[2:length_lines]:
            colum0 = (line.replace("%"," ")).split()[0]
            colum1 = (line.replace("%"," ")).split()[1]
            #print "COL=",colum0
            if colum0 == "total":
                #print "the final line breaks"
                break
            else:
                rsym = float(colum1)
            if rsym < 0.0:
                ...
                #print "ELSE=",line.replace("%"," ").split()
                dsave = float((line.replace("%"," ")).split()[0])
        # The lowest shell Rsymm
        lowest_res_rsym=float((lines_eachdata[2].replace("%"," ")).split()[1])
        # Overall Rsymm
        total_rsym=float((lines_eachdata[length_lines-1].replace("%"," ")).split()[1])
        # Data name
        dname = "INPUT_FILE=%s"% lines_eachdata[0].split()[6]
        # dmin for this data (maximum resolution in the initial scale)
        if bad_flag == False:
            dmin_this_data = float((lines_eachdata[length_lines-2].replace("%"," ")).split()[0])
            #print "DMIN=",dmin_this_data
...
if __name__=="__main__":
    ...
    outfile=sys.argv[1].replace(".LP",".plt")
    ...
    #print ac.determineResolution()
    #ac.makePlot()
    #ac.checkCChalf()

    #ac.setNbin(20)
    datalist = ac.makeDataList()
    for dname, dmin, lowr, ovr in datalist:
        print "%8.2f %8.2f %s"%(dmin, ovr, dname)
    #ac.choose(dmin_thresh = 3.5, overall_r_thresh = 250.0)
    #ac.choose(r_thresh=10,r_total_thresh=100)
    #ac.choose(r_thresh=10,r_total_thresh=150)
    #ac.choose(r_thresh=10,r_total_thresh=200)
    #ac.choose(r_thresh=15,r_total_thresh=100)
    #ac.choose(r_thresh=15,r_total_thresh=150)
    #ac.choose(r_thresh=15,r_total_thresh=200)
    #ac.choose(r_thresh=20,r_total_thresh=100)
    #ac.choose(r_thresh=20,r_total_thresh=150)
    #ac.choose(r_thresh=20,r_total_thresh=200)
