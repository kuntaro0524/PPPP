...
class XDSReporter():
    def __init__(self, root_path, logfile="CORRECT.LP"):
        ...
        #self.corrlp_list,self.path_list=self.dp.findTarget(logfile)
        self.log_list,self.path_list=self.dp.findTarget(logfile)
        self.libspg = LibSPG.LibSPG()

    def makeLogText(self,logfile="correct.txt"):
        # Log file
        logf=open(logfile,"w")
    ...
    def makeHTML(self,htmlfile="correct.html",prefix="result",figdpi=60,skipMulti=True):
        # Log file
        html_path = "%s/%s" % (self.rootdir, htmlfile)
        logf=open(html_path,"w")
        #exec_dir = os.path.abspath("../../")
        logf.write("%s\n" %  head)
        ...
        for corrlp in self.log_list:
            ...
            relpath = os.path.relpath(corrlp, self.rootdir)
...
if __name__ == "__main__":
    xdsr = XDSReporter(sys.argv[1])
    xdsr.makeHTML(figdpi=65,prefix="correct_ana")
    #xdsr.makePlots()
    #xdsr.makePlots()
