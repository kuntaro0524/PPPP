...
class DirectoryProc():
    ...
    def findDirsWithName(self, include_string):
        dires = self.findDirs()
        rtn_list = []
        for dirname in dires:
            #print dirname, include_string
            if dirname.rfind(include_string) != -1:
                rtn_list.append(dirname)
    ...
    def findFileInDirList(self, dir_list_file, filename):
        dir_list = open(dir_list_file, "r").readlines()
