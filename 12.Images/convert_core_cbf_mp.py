...
def run(param):
  masterfile,startnum,endnum,outf=param
  print "Processing %s %d %d"%(masterfile,startnum,endnum)
  frameno=10
  data = None
  sel = None
  nimage=0
...
for line in lines:
	...
	root_dir=fname[:jjj]
	cbf_dir="%s/data/cbf-tail/"%root_dir

	if os.path.exists(cbf_dir)==False:
		os.mkdir(cbf_dir)
	cbfile="%s/head-%d-%d.cbf"%(cbf_dir,core_start,core_end)

	compo=master_file,core_start,core_end,cbfile
	conds_list.append(compo)

easy_mp.pool_map(fixed_func=lambda n: run(n), args=conds_list, processes=8)
