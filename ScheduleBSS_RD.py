...
class ScheduleBSS:
	...
	def make(self,sch_file):
		...
		if self.skip_option!="":
			ofile.write("Advanced skipped point:%s"%self.skip_option)
...
if __name__=="__main__":
	...
	print "DIR THICKNESS OX OY OZ HX HY HZ VX VY VZ ADVSTEP[um] DATANAME"
	...
	adstep=float(sys.argv[12])

        slist,elist,flen,slen=vecg.yattane(ovec,hvec,vvec,nf,ns)

	j=0
       	for i in range(0,len(slist)):
		print slist[i],elist[i]
		t.setDir(sys.argv[1])
		t.setCameraLength(130)
		t.setAttThickness(200)
		t.stepAdvancedRD(slist[i],elist[i],adstep,1,startphi,stepphi,interval)
		t.setDataName(dname)
		for skip in skip_string:
			t.setAdvSkip(skip)
			t.setOffset(j)
			t.make("tmp%03d.sch"%j) 
			j+=1
